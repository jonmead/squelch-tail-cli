 ...
 *
 * The display application is expected at SQUELCH_DISPLAY_HOME, which
 * defaults to the sibling directory ../squelch-tail-display relative to
 ...
 *   SQUELCH_DISPLAY_HOME    path to squelch-tail-display (default: ../squelch-tail-display)
...
const _DISPLAY_HOME = path.resolve(_CLI_ROOT, '..', 'squelch-tail-display');
...
class PygameDisplayPlugin {
    ...
    _readEnv() {
        const home = process.env.SQUELCH_DISPLAY_HOME || _DISPLAY_HOME;
    ...
    _launch() {
        ...
        if (!existsSync(mainPy)) {
            ...
            this.log.error?.(`Set SQUELCH_DISPLAY_HOME to the squelch-tail-display directory.`);
            return;
        }
